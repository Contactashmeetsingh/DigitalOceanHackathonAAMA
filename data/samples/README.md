...
Real genome exports are intentionally excluded from Git. For local parser
regression checks, use open-consent files published by the Harvard Personal
Genome Project and keep the downloaded bytes under this directory (the
`.gitignore` excludes `*.txt` and `*.csv`). Do not use a teammate’s private
genome as a demo fixture.
